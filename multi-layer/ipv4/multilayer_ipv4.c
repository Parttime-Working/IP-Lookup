...
Layer0_struct *layer0_table, *toplayer_table;
...
int layerArray[10] = {0};
int num[65536] = {0};
int num_entry = 0, num_insert = 0, num_query = 0, num_delete = 0,
    layer0_count = 0;
...
struct Upper_BTreeNode *upper_createNode(unsigned int ip,
                                         unsigned int len,
                                         struct Upper_BTreeNode *child)
{
    ...
    newNode->item = (Layer0_struct *) malloc((MAX + 1) * sizeof(Layer0_struct));
...
void set_table(char *file_name)
{
    ...
    segment_16 =
        (struct segment_array *) malloc(65536 * sizeof(struct segment_array));
...
void set_query(char *file_name)
{
    ...
    clock1 = (unsigned long long int *) malloc(num_query *
...
void insertValue(unsigned int item,
                 unsigned int len,
                 int pos,
                 ...
                 struct btreeNode *child)
{
    ...
    node->item[j + 1].ip = item;
    node->item[j + 1].len = len;
    node->link[j + 1] = child;
    node->count++;
}

void splitNode(unsigned int item,
               unsigned int *pval,
               unsigned int len,
               unsigned int *pval2,
               int pos,
               ...
               struct btreeNode **newNode)
{
    ...
    *newNode = (struct btreeNode *) malloc(sizeof(struct btreeNode));
    (*newNode)->item =
        (Layer0_struct *) malloc((MAX + 1) * sizeof(Layer0_struct));
    j = median + 1;
    while (j <= MAX) {
    ...
    if (pos <= MIN) {
        insertValue(item, len, pos, node, child);
    } else {
        insertValue(item, len, pos - median, *newNode, child);
    }
...
int setNodeValue(unsigned int item,
                 unsigned int len,
                 unsigned int *pval,
                 unsigned int *pval2,
                 struct btreeNode *node,
                 ...
                 int flag)
{
    ...
    if (flag == 0) {
        if (item < node->item[1].ip) {
            ...
            for (pos = node->count; (item < node->item[pos].ip && pos > 1);
                 pos--)
                ;
        ...
        if (item <= node->item[1].ip) {
            ...
            if (item == node->item[1].ip) {
                if (len >= node->item[1].len) {
                    ...
                    Replcae_Layer0++;
                    replace_flag = 1;
                    insert_Btree(1, node->item[1].ip, node->item[1].len, 1,
                                 allBtree_root->count, allBtree_root, NULL, 0);
                    node->item[1].ip = item;
                    node->item[1].len = len;
                    return 2;
                } else {
            ...
            if (item > node->item[node->count].ip)
                if (cover(node->item[node->count].ip,
                          node->item[node->count].len, item, len)) {
                    Insert2Up++;
                    Replcae_Layer0++;
                    replace_flag = 1;
                    insert_Btree(1, node->item[node->count].ip,
                                 node->item[node->count].len, 1,
                                 allBtree_root->count, allBtree_root, NULL, 0);
            ...
            while (idxl <= idxr) {
                ...
                if (item == node->item[mid].ip) {
                    if (len >= node->item[mid].len) {
                        ...
                        Replcae_Layer0++;
                        replace_flag = 1;
                        insert_Btree(1, node->item[mid].ip, node->item[mid].len,
                                     1, allBtree_root->count, allBtree_root,
                                     NULL, 0);
                ...
                if (idxl > idxr) {
                    if (cover_case == 0) {
                        if (cover(item, len, node->item[mid].ip,
                                  node->item[mid].len)) {
                            Insert2Up++;
                            Cover_layer0++;
                            return 0;
                        } else if (cover(node->item[mid - 1].ip,
                                         node->item[mid - 1].len, item, len)) {
                            Insert2Up++;
                            Replcae_Layer0++;
                            replace_flag = 1;
                            insert_Btree(1, node->item[mid - 1].ip,
                                         node->item[mid - 1].len, 1,
                                         allBtree_root->count, allBtree_root,
                                         NULL, 0);
                        ...
                        if (cover(item, len, node->item[mid + 1].ip,
                                  node->item[mid + 1].len)) {
                            Insert2Up++;
                            Cover_layer0++;
                            return 0;
                        } else if (cover(node->item[mid].ip,
                                         node->item[mid].len, item, len)) {
                            Insert2Up++;
                            Replcae_Layer0++;
                            replace_flag = 1;
                            insert_Btree(
                                1, node->item[mid].ip, node->item[mid].len, 1,
                                allBtree_root->count, allBtree_root, NULL, 0);
                            node->item[mid].ip = item;
                            node->item[mid].len = len;
                            return 2;
    ...
    if (setNodeValue(item, len, pval, pval2, node->link[pos], child, flag) ==
        1) {
        if (node->count < MAX) {
            insertValue_flag = 1;
            insertValue(*pval, *pval2, pos, node, *child);
            return 2;
        } else {
            splitNode_flag = 1;
...
void btree_insertion(struct btreeNode **B_root,
                     unsigned int item,
                     unsigned int len,
                     int insert_type)
{
    ...
    if (flag == 1) {
        *B_root = createNode(*B_root, i, i2, child);
        ...
        insert_Btree(1, item, len, 1, allBtree_root->count, allBtree_root, NULL,
                     0);
    }
}
int BtreeSearch(struct btreeNode *myNode, unsigned int ip, int idxl, int idxr)
{
    int mid = 0;
    int bndl = 0;
    while (idxl <= idxr) {
        mem_access++;
        mid = (idxl + idxr) / 2;
        if (is_match(ip, myNode->item[mid].ip,
                     myNode->item[mid]
                         .len)) {  // is_match( ip, layer0_table[mid].ip,
                                   // layer0_table[mid].len) ){
                                   // //layer0_table[mid].up_bound >= ip &&
                                   // layer0_table[mid].ip <= ip
            match_rule++;
            layer0_found++;
            return 1;
        }
...
void Build_Seg()
{
    ...
    for (i = 0; i < 65536; i++) {
        ...
        segement_table[i].list_table = (struct ENTRY *) malloc(
            (max_segment_size + 1) * sizeof(struct ENTRY));
    }
    for (int i = 0; i < layer0_count; i++) {
        if (layer0_table[i].len > 16) {
            unsigned int ip = layer0_table[i].ip >> 16;
            segement_table[ip].list_table[segement_table[ip].num].len =
                layer0_table[i].len;
            segement_table[ip].list_table[segement_table[ip].num].ip =
                layer0_table[i].ip;
...
void build_segment_create(int pos)
{
    for (int i = 0; i < segement_table[pos].num; i++) {
        btree_insertion(&segement_table[pos].B_root,
                        segement_table[pos].list_table[i].ip,
                        segement_table[pos].list_table[i].len, 0);
    }
}
////////////////////////////////////////////////////////////////////////////////////////////////////////
void addValToNode(unsigned int ip,
                  unsigned int len,
                  int pos,
                  struct Upper_BTreeNode *node,
                  struct Upper_BTreeNode *child)
{
    int j = node->count;
    while (j > pos) {
...
void upper_splitNode(unsigned int ip,
                     unsigned int *pval,
                     unsigned int len,
                     unsigned int *pval2,
                     int pos,
                     ...
                     struct Upper_BTreeNode **newNode)
{
    ...
    *newNode =
        (struct Upper_BTreeNode *) malloc(sizeof(struct Upper_BTreeNode));
    (*newNode)->item =
        (Layer0_struct *) malloc((MAX + 1) * sizeof(Layer0_struct));
    for (int i = 0; i < MAX + 1; i++) {
        (*newNode)->nextlevel_root[i] = NULL;
    ...
    if (pos <= MIN) {
        addValToNode(ip, len, pos, node, child);
    } else {
        addValToNode(ip, len, pos - median, *newNode, child);
    }
    *pval = node->item[node->count].ip;
    *pval2 = node->item[node->count].len;
...
int upper_setValueInNode(unsigned int ip,
                         unsigned int len,
                         unsigned int *pval,
                         unsigned int *pval2,
                         struct Upper_BTreeNode *node,
                         struct Upper_BTreeNode **child)
{
    ...
    if (upper_setValueInNode(ip, len, pval, pval2, node->linker[pos], child)) {
        if (node->count < MAX) {
            addValToNode(*pval, *pval2, pos, node, *child);
        } else {
            upper_splitNode(*pval, pval, *pval2, pval2, pos, node, *child,
                            child);
            return 1;
        }
    }
...
void insertion(unsigned int ip,
               unsigned int len,
...
void search_Btree(int level,
                  struct Upper_BTreeNode *myNode,
                  unsigned int ip,
                  int idxl,
                  int idxr)
{
    ...
    while (idxl <= idxr) {
        ...
        if (is_match(ip, myNode->item[mid].ip, myNode->item[mid].len)) {
            ...
            if (myNode->nextlevel_root[mid] != NULL) {
                search_Btree(level + 1, myNode->nextlevel_root[mid], ip, 1,
                             myNode->nextlevel_root[mid]->count);
                return;
            } else
                return;
        }
        ...
        if (idxl > idxr) {
            ...
            if (myNode->linker[bndl] != NULL) {
                ...
                for (int i = 0; i < current_root->eset_num; i++) {
                    if (is_match(ip, current_root->eset[i].ip,
                                 current_root->eset[i].len)) {
                        match_flag = 1;
...
void Searching(struct Upper_BTreeNode *myNode,
               unsigned int ip,
               int len,
               int idxl,
               int idxr,
...
void traversal_build_tree(int level,
                          struct Upper_BTreeNode *current_root,
                          Top_struct *current,
                          int num)
{
    int i;
        ...
        for (i = 0; i < num; i++) {
            if (current[i].nextlevel != NULL) {
                ...
                for (int j = 0; j < current[i].num; j++) {
                    insertion(current[i].nextlevel[j].ip,
                              current[i].nextlevel[j].len, NULL);
                }
                Searching(current_root, current[i].ip, current[i].len, 1,
                          top_count, top_root);
                current[i].nextlevel_root = top_root;
                traversal_build_tree(level + 1, current[i].nextlevel_root,
                                     current[i].nextlevel, current[i].num);
            }
        }
    }
}
...
{
    Upper_Btree_root =
        (Top_struct *) malloc((toplayer_array[1] + 1000) * sizeof(Top_struct));
    printf("root_num%d\n", root_num);
    ...
    push_prefix(root);
    printf("num_entry: %d\n", num_entry);
    Top_struct **build_queue =
        (Top_struct **) malloc((num_entry + 10000) * sizeof(Top_struct *));
    count_n = 0;
    int max_index = 0;
    int q_size = 0;
    ...
    for (int j = 0; j < toplayer_array[1]; j++) {
        count_n = 0;
        Upper_Btree_root[j].nextlevel =
            (Top_struct *) malloc(3000 * sizeof(Top_struct));
        ...
        build_btree(Upper_Btree_root[j].trie_node,
                    Upper_Btree_root[j].nextlevel);
...
void insert_Btree(int level,
                  unsigned int ip,
                  int len,
                  int idxl,
                  int idxr,
                  struct Upper_BTreeNode *current,
                  struct Upper_BTreeNode *pre_node,
                  int pre_index)
{
    int mid = 0;
    int bndl = 0;
    ...
    while (idxl <= idxr) {
        ...
        if (idxl > idxr) {
            n_num = current->count;
            if (bndl != 0 && cover(current->item[bndl].ip,
                                   current->item[bndl].len, ip, len)) {
                if (current->nextlevel_root[bndl] != NULL) {
                    insert_Btree(level + 1, ip, len, 1,
                                 current->nextlevel_root[bndl]->count,
                                 current->nextlevel_root[bndl], current, bndl);
                    return;
                } else {
                    case2++;
                    top_root = NULL;
                ...
                if (bndl + 1 <= n_num &&
                    cover(ip, len, current->item[bndl + 1].ip,
                          current->item[bndl + 1].len)) {
                    if (current_root->eset_num == 0) {
                        current_root->eset = (Layer0_struct *) malloc(
                            1600 * sizeof(Layer0_struct));
...
void inition()
{
    Set_Layer(root);
    toplayer_table =
        (Layer0_struct *) malloc(num_entry * sizeof(Layer0_struct));
    Set_top(root, 1);
    top_layer_num = toplayer_array[1];
    build();
    layer0_table =
        (Layer0_struct *) malloc(layerArray[1] * sizeof(Layer0_struct));
}
...
void lookup_performance()
{
    ...
    for (int j = 0; j < 100; j++) {
        for (int i = 0; i < num_query; i++) {
            ...
            if (segement_table[ip_temp].B_root == NULL) {
                ...
                int search_flag =
                    BtreeSearch(segement_table[ip_temp].B_root, query[i], 1,
                                segement_table[ip_temp].B_root->count);
                if (!search_flag) {
                    match_flag = 0;
                    search_Btree(1, allBtree_root, query[i], 1,
                                 allBtree_root->count);
                    if (match_flag) {
                        upper_match++;
                        match_rule++;
            ...
            if (mem_access - temp_access > max_access)
                max_access = mem_access - temp_access;
            end = rdtsc();
    ...
    // printf("match_rule = %d , no_match=%d , upper_match = %d ,layer0_found =
    // %d ,segment_16_found = %d
...
void insert_performance()
{
    ...
    for (int i = 0; i < num_insert; i++) {
        ...
        if (insert_table[i].len > 16)
            btree_insertion(&segement_table[ip_temp].B_root, insert_table[i].ip,
                            insert_table[i].len, 1);  // segment
