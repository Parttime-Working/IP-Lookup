...
void search(struct ENTRY array){
	...
	{	
		if(big_box[(int)((array.ip) >> 16)].down != NULL){
			...
			//printf("index= %u\n", index);
			if(answer != 0){
				int posfix_ip = ((array.ip) >> 16);
				for(j = index; j >= 0; --j){
					if(big_box[posfix_ip].down[j].exist == 1)
						++count;
				}
			}
		}
...
int find_max_height(btrie ptr){
	if(ptr == NULL)
		return 0;
	max(find_max_height(ptr->left) + 1, find_max_height(ptr->right) + 1);
}
////////////////////////////////////////////////////////////////////////////////////
...
int main(int argc,char *argv[]){
	...
	printf("16 x compress build time = %d\n", (end - begin)/num_entry);
