...
void count_active_node(btrie ptr){
	if(ptr == NULL)
		return;
	if(ptr -> port != 256)
		active_node_num++;
	count_active_node(ptr -> left);
	count_active_node(ptr -> right);
}

int count_sub_active_node(btrie ptr){
	if(ptr == NULL)
		return 0;
	if(ptr -> port != 256){
		return 1 + count_sub_active_node(ptr -> left) + count_sub_active_node(ptr -> right);
	}
	else
	{
		return count_sub_active_node(ptr -> left) + count_sub_active_node(ptr -> right);
	}
	
}

int split_num = 0;
int split_size, now_split_size;

void Post_split(btrie ptr){

	if(ptr == NULL)
		return;
	
	Post_split(ptr -> left);
	Post_split(ptr -> right);

	int this_size = count_sub_active_node(ptr);

	if(this_size == now_split_size){
		if(now_split_size == split_size){
			//printf("now split size: %d, split size : %d, this size : %d\n", now_split_size, split_size, this_size);
			++split_num;
		}
			
		now_split_size = split_size;
		if(ptr -> parent != NULL){
			...
			{
				printf("this  could not occur\n");
			}
		}
		
	}
	else if(this_size < now_split_size && count_sub_active_node(ptr->parent) > now_split_size){
		if(now_split_size == split_size){
			++split_num;
		}
		now_split_size = now_split_size - this_size;
		if(ptr -> parent != NULL){
			if(ptr -> parent -> right == ptr){
				ptr -> parent -> right == NULL;
			}
...
int main(int argc,char *argv[]){
	...
	now_split_size  = atoi(argv[2]);
	//////////////

	count_active_node(root);
	Post_split(root);
	printf("active_node_num : %d, split_num : %d\n", active_node_num, split_num);
	return 0;
	/*
