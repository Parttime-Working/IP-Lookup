...
int split_num = 0;
int split_size = 4;
void Subtree_split(btrie ptr){

	if(ptr == NULL)
		return;
	
	Subtree_split(ptr -> left);
	Subtree_split(ptr -> right);

	if(count_sub_active_node(ptr) >= (split_size+1) / 2 && count_sub_active_node(ptr->parent) > split_size){
		if(ptr -> parent != NULL){
